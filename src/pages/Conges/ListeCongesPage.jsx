...
const ListeCongesPage = () => {
  ...
  const getStatusBadge = (statut) => {
    ...
    return (
      <span className={`badge ${styles[statut].class} d-flex align-items-center gap-1`}>
  ...
  const fetchConges = async () => {
    ...
    try {
      ...
      console.log('[FETCH] Conges state updated:', response.data.length, 'items');
    } catch (error) {
  ...
  return (
    <div className="card border">
            ...
            {conges.length === 0 && (
              <div className="text-center py-5 bg-light rounded-3">
                ...
                <p className="small text-muted">
                  Données reçues: {JSON.stringify(conges)}
                </p>
              </div>
            )}
          </div>
      ...
      {/* Modal for updating conge status */}
      <Modal
        isOpen={showModal}
